(() => {
  ...
  const PAGE_SIZE = 100
  const STATIC_VERSION = '3.4.0-shell-10'
  ...
  function cuttingSection(tool) {
    ...
    return profiles.map(profile => `<article class="cutting-profile" data-profile-id="${escapeHtml(profile.id)}">
      ...
      <div class="cutting-values">
        ...
        ${profile.coolant_condition && profile.coolant_condition !== 'unknown' ? `<div><span>Coolant</span><b>${escapeHtml(humanize(profile.coolant_condition))}</b></div>` : ''}
      </div>
